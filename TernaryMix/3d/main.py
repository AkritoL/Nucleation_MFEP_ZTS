...
mean_uc = 4*jnp.pi*jnp.power(rf, 3)/3 #The mean concentration of component c
...
def generator(R, ratioAB):  #phase a is above the interface and R is the intermediate radus of the droplets
    ...
    '''
    xc = (4*jnp.pi*jnp.power(rf,3)/3 - 4*jnp.pi*jnp.power(R,3)/3) / (jnp.power(l,3) - 4*jnp.pi*jnp.power(R,3)/3)
    ...
    if args.NUCLEATION_MODE == "homogeneous":
        if args.CASE == "AsymmetricCase" and gamma_ca > 1e-2:
            ...
            d = (rx - l/2)**2 + (ry - l/2)**2 + (rz - 3*ratioAB/2)**2
            sphere_check = d<=R**2
            semicubic_check = rz<ratioAB

            uc = jnp.where(sphere_check, 1, uc)
            ub = jnp.where(semicubic_check, mean_uc - xc, ub)
            ub = jnp.where(sphere_check, 0, ub)
            ua = jnp.where(semicubic_check, 1 - mean_uc, ua)
            ...
            d = (rx - l/2)**2 + (ry - l/2)**2 + (rz - ratioAB/2)**2
            sphere_check = d<=R**2
            semicubic_check = rz>ratioAB

            uc = jnp.where(sphere_check, 1, uc)
            ua = jnp.where(semicubic_check, mean_uc - xc, ua)
            ua = jnp.where(sphere_check, 0, ua)
            ub = jnp.where(semicubic_check, 1 - mean_uc, ub)
            ub = jnp.where(sphere_check, 0, ub)
        ...
        d = (rx - l/2)**2 + (ry - l/2)**2 + (rz - ratioAB)**2
        sphere_check = d<=R**2
        semicubic_check = rz>ratioAB
    ...
    noise = jax.random.normal(key, shape=(N, N, N), dtype=jnp.float64) * 1e-8
    ua = ua + noise
    ub = ub + noise
    uc = uc + noise
    '''
...
def pde_solver_FFT(h, ua, ub, uc):
    ua_hat = jnp.fft.fftn(ua, axes=(1,2,3))
    ub_hat = jnp.fft.fftn(ub, axes=(1,2,3))
    uc_hat = jnp.fft.fftn(uc, axes=(1,2,3))

    S = ua**2 + ub**2 +uc**2
    P = jnp.power(1-ua, 2)*jnp.power(1-ub, 2)*jnp.power(1-uc, 2)

    mu_a_hat = jnp.fft.fftn(cal_ChemicalPotential(0, ua, S, P), axes=(1,2,3))
    mu_b_hat = jnp.fft.fftn(cal_ChemicalPotential(1, ub, S, P), axes=(1,2,3))
    mu_c_hat = jnp.fft.fftn(cal_ChemicalPotential(2, uc, S, P), axes=(1,2,3))
    ...
    ua_new = jnp.fft.ifftn(ua_hat_new, axes=(1,2,3)).real
    ub_new = jnp.fft.ifftn(ub_hat_new, axes=(1,2,3)).real
    uc_new = jnp.fft.ifftn(uc_hat_new, axes=(1,2,3)).real
...
def reparameterize(images):
    ...
    dist = jnp.sum(diff**2, axis=(1, 2, 3))
    ...
    return jax.vmap(interplate, in_axes=(None, 0), out_axes=1)(images.reshape((image_num, N**3)), jnp.arange(N**3)).reshape(images.shape)
...
def main_loop(ua, ub, uc, h, ratioAB):
    ...
    print(f"3d, {args.CASE}, nucleation_mode = {args.NUCLEATION_MODE}")
...
if args.PLOT_CONCENTRATION_RGB: 
    # here we clip the cubic at the middle, only show the middle clip of whole concentration
    m = int(N/2)
    mid_ua = (ua[:, m, :, :] + ua[:, :, m, :] + ua[:, m-1, :, :] + ua[:, :, m-1, :])/4
    mid_ub = (ub[:, m, :, :] + ub[:, :, m, :] + ub[:, m-1, :, :] + ub[:, :, m-1, :])/4
    mid_uc = (uc[:, m, :, :] + uc[:, :, m, :] + uc[:, m-1, :, :] + uc[:, :, m-1, :])/4
    plot_RGB(mid_ua[0, : ,:], mid_ub[0, : ,:], mid_uc[0, : ,:], name="Initial Concentration", file_path=file_path, savefig=True, show=False)
    plot_RGB(mid_ua[critical_index, : ,:], mid_ub[critical_index, : ,:], mid_uc[critical_index, : ,:], name="Critical Concentration", file_path=file_path, savefig=True, show=False)
    plot_RGB(mid_ua[-1, :, :], mid_ub[-1, :, :], mid_uc[-1, :, :], name="Final Concentration", file_path=file_path, savefig=True, show=False)
